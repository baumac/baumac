...
const cmdEnvVars = `POSTGRES_PORT=${process.env.POSTGRES_PORT ?? 5432} POSTGRES_USERNAME=${process.env.POSTGRES_USERNAME ?? 'postgres'} POSTGRES_PASSWORD=${process.env.POSTGRES_PASSWORD ?? 'postgres'} POSTGRES_DB=${process.env.POSTGRES_DB ?? 'ratings'}`
